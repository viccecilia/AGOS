...
def main() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        report = SpatialIntelligenceEngine(
        ...
        assert report["report_id"] == "SPATIAL_INTELLIGENCE_REPORT"
        assert report["round_id"] == "ROUND-GLOBAL-008"
        assert report["status"] == "spatial_intelligence_ready"
        spatial = report["spatialIntelligence"]
        assert len({item["location_name"] for item in spatial}) >= 10
        ...
        for item in spatial:
            ...
            assert item["demand_heat_score"] >= 0
            assert item["human_review_required"] is True
            assert item["sample_data_only"] is True
            assert item["gps_dispatch_enabled"] is False
            assert item["automatic_driver_contact_enabled"] is False
            assert item["auto_publish_allowed"] is False
            assert item["auto_reply_allowed"] is False
            assert item["write_api_allowed"] is False

        location_types = {item["location_type"] for item in spatial}
        assert "airport" in location_types
        assert "city" in location_types
        assert "attraction" in location_types
        assert "event_venue" in location_types

        summary = report["spatialIntelligenceSummary"]
        assert summary["spatial_intelligence_ready"] is True
        assert summary["location_count"] >= 10
        assert summary["gps_dispatch_enabled"] is False
        assert summary["automatic_driver_contact_enabled"] is False
        assert summary["automatic_vehicle_contact_enabled"] is False
        assert summary["all_items_human_review_required"] is True
        assert summary["write_api_allowed"] is False

        assert report["locationMarketHeatmap"]
        assert report["spatialDemandRanking"]

        for output_name in [
            "SPATIAL_INTELLIGENCE_REPORT.json",
            "spatial_intelligence.json",
            "location_market_heatmap.json",
            "spatial_demand_ranking.json",
            "spatial_intelligence_summary.json",
        ]:
            path = spatial_dir / output_name
            assert path.exists(), f"missing output: {output_name}"
            json.loads(path.read_text(encoding="utf-8"))

    print("spatial_intelligence_engine_smoke_test passed")
