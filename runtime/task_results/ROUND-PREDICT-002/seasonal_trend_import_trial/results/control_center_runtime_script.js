const state = JSON.parse(document.getElementById("project-state").textContent);
    ...
    function renderWarRoomGrowth() {
      ...
      const seasonalTrendSummaryRow = '<div class="feed-item"><time>TRIAL</time><div class="feed-main"><strong>Seasonal Trend Import Trial / ' + esc(seasonalTrendSummary.data_source_status || 'sample import') + '</strong><span>Imported keywords: ' + esc(seasonalTrendSummary.imported_keyword_count || 0) + ' / Matched seasonal signals: ' + esc(seasonalTrendSummary.matched_seasonal_signals || 0) + ' / High confidence: ' + esc(seasonalTrendSummary.high_confidence_matches || 0) + '</span><span>Market heatmap rows: ' + esc(seasonalTrendSummary.market_heatmap_rows || 0) + ' / Demand types: ' + esc(seasonalTrendSummary.demand_type_count || 0) + ' / Mobility pain points: ' + esc(seasonalTrendSummary.mobility_pain_point_count || 0) + '</span><span>API status: ' + esc(seasonalTrendSummary.api_status || 'real_google_trends_api_not_connected') + ' / Write action status: ' + esc(seasonalTrendSummary.write_action_status || 'blocked') + ' / Sample data only: ' + esc(seasonalTrendSummary.sample_data_only || false) + '</span></div><span class="badge b-amber">sample</span></div>';
      ...
      el('runtime-seasonal-trend-import-trial').innerHTML = ((wr.seasonalTrendMatches || []).length ? seasonalTrendSummaryRow + seasonalTrendMatchRows.concat(seasonalHeatRows, seasonalDemandRows, seasonalPainRows, seasonalNoiseRows).join('') : '') || '<div class="feed-item"><time>WAIT</time><div class="feed-main"><strong>Seasonal Trend Import Trial pending</strong><span>AGOS will read local Google Trends-style CSV/JSON samples, map keywords to seasons, build market heat, and keep real Google Trends API disconnected.</span></div><span class="badge b-amber">pending</span></div>';
