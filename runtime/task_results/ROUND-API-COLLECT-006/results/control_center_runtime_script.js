    ...
    const statusLabel = {
      ...
      blocked: "髦ｻ蝪・,
      not_initialized: "Git 譛ｪ蛻晏ｧ句喧",
      local_initialized: "譛ｬ蝨ｰ Git 蟾ｲ蛻晏ｧ句喧",
      clean: "蟾ｲ荳贋ｼ Git",
      dirty: "譛ｬ蝨ｰ譛画隼蜉ｨ"
    };
    const statusClass = {
    ...
    function renderOverview() {
      el("project-title").textContent = `${state.project.name} 謗ｧ蛻ｶ荳ｭ蠢チ;
      ...
      el("current-round-badge").textContent = `蠖灘燕 ${state.project.currentRound}`;
    ...
    function renderRuntimeApiBanner() {
      ...
      banner.innerHTML =
        ...
        ' ﾂｷ last_api_ping: ' + esc(runtimeApiState.last_api_ping) +
        ' ﾂｷ last_runtime_action: ' + esc(runtimeApiState.last_runtime_action) +
        (runtimeApiState.last_error ? ' ﾂｷ last_error: ' + esc(runtimeApiState.last_error) : '');
    }
    ...
    async function fetchRuntimeApiStatus() {
      ...
      try {
        ...
        runtimeApiState.last_error = "Runtime API 譛ｪ霑樊磁縲りｯｷ霑占｡鯉ｼ嗔ython services/runtime_api_server.py";
        renderRuntimeApiBanner();
        return false;
      }
    }
    ...
    async function submitRuntimeCorrection() {
      const payload = {
        ...
        correction_type: el("runtime-correction-type")?.value || "髞呵ｯｯ譛ｺ莨壼愛譁ｭ",
    ...
    function renderWarRoomGrowth() {
      ...
      el('war-room-control-panel').innerHTML =
        ...
        '<div class="runtime-kv"><span>迥ｶ諤・/span><strong>' + esc(runtimeStatus) + '</strong></div>' +
        '<div class="runtime-kv"><span>蠖灘燕 Cycle</span><strong>' + esc(control.currentCycle || wr.currentCycle || 'CYCLE-001') + '</strong></div>' +
        '<div class="runtime-kv"><span>蠖灘燕闃らせ</span><strong>' + esc(currentStage) + '</strong></div>' +
        '<div class="runtime-kv"><span>蟾ｲ霑占｡梧慮髣ｴ</span><strong>' + esc(wr.runtimeElapsed || '00:00:00') + '</strong></div>' +
        ...
        '<div class="runtime-actions"><button class="primary" type="button" data-runtime-action="start">蜷ｯ蜉ｨ</button><button class="danger" type="button" data-runtime-action="stop">蛛懈ｭ｢</button></div>';
      ...
      renderRuntimeApiBanner();
      el('runtime-drift-banner').textContent = wr.isLearningDrifting ? 'AGOS 豁｣蝨ｨ蟄ｦ豁ｪ鬟朱勦・夐怙隕∽ｺｺ蟾･螳｡譬ｸ譬ｷ萓狗ｻ楢ｮｺ・檎ｦ∵ｭ｢謚頑ｷ萓句ｽ捺・逵溷ｮ槫渚鬥医・ : 'AGOS 蟄ｦ荵譁ｹ蜷第ｭ｣蟶ｸ縲・;
      ...
      el('war-room-feed').innerHTML = (wr.warRoomFeed || []).map(item =>
        '<div class="feed-item"><time>' + esc(item.time) + '</time><div class="feed-main"><strong>' + esc(item.type) + '</strong><span>' + esc(item.sourcePlatform) + ' ﾂｷ ' + esc(item.country) + ' ﾂｷ ' + esc(item.language) + ' ﾂｷ ' + esc(item.audience) + ' ﾂｷ 諠・ｻｪ ' + esc(item.emotion) + '</span></div><span class="badge ' + (item.status === 'risk' || item.status === 'needs_human_review' ? 'b-amber' : 'b-blue') + '">' + esc(item.aiAction) + '</span></div>'
      ).join('');
      el('war-room-jag-workspace').innerHTML =
        ...
        '<div class="runtime-stat"><span>蠖灘燕逶ｮ譬・ｸょ惻</span><strong>' + esc(workspace.targetMarket || workspace.focusMarket) + '</strong></div>' +
        '<div class="runtime-stat"><span>蠖灘燕驥咲せ蟷ｳ蜿ｰ</span><strong>' + esc((workspace.focusPlatforms || []).join(', ')) + '</strong></div>' +
        '<div class="runtime-stat"><span>蠖灘燕驥咲せ逞帷せ</span><strong>' + esc(workspace.focusPainPoint || '蠕・・譫・) + '</strong></div>' +
        '<div class="runtime-stat"><span>鬟朱勦迥ｶ諤・/span><strong>' + esc(workspace.riskStatus || 'needs_review') + '</strong></div>' +
        '</div><div style="height:8px"></div><div class="runtime-stat"><span>莉頑律逶ｮ譬・/span><strong>' + esc(workspace.todayGoal) + '</strong></div>';
      ...
      el('war-room-growth-cycles').innerHTML = (wr.growthCycles || []).map(cycle => {
        const steps = (cycle.timeline || []).map(step => '<span class="mini-step ' + esc(step.status) + '">' + esc(step.node) + ' ' + (step.status === 'done' ? '笨・ : esc(step.status)) + '</span>').join('');
        return '<details class="runtime-details"><summary><span>' + esc(cycle.cycleId) + '</span><span class="mini-steps">' + steps + '</span><span>' + esc(cycle.status) + '</span></summary><div class="runtime-detail-body"><div><strong>謾ｶ髮・・螳ｹ</strong>・・ + esc((cycle.collectedQuestions || []).join(' / ')) + '</div><div><strong>蛻・梵扈捺棡</strong>・・ + esc(cycle.analysisConclusion) + '</div><div><strong>蟄ｦ荵扈捺棡</strong>・・ + esc(cycle.learningResult || '') + '</div><div><strong>遲也払蜿伜喧</strong>・・ + esc(cycle.strategyChange || cycle.generatedStrategy) + '</div></div></details>';
      }).join('');
      el('war-room-growth-stages').innerHTML = (wr.growthStages || []).map(stage =>
        '<details class="strategy-stage"><summary>' + esc(stage.cycleRange) + '<br>' + esc(stage.name) + '</summary><div><strong>髦ｶ谿ｵ逶ｮ譬・/strong><br>' + esc(stage.stageGoal) + '<br><strong>蟄ｦ荵謨ｰ驥・/strong> ' + esc(stage.learningCount ?? stage.collectionCount) + '<br><strong>鬮倅ｻｷ蛟ｼ髣ｮ鬚・/strong><br>' + esc(stage.highValueQuestion || '蠕・噺髮・) + '<br><strong>譛蠑ｺ遲也払</strong><br>' + esc(stage.strongestStrategy || '蠕・ｽ｢謌・) + '<br><strong>譛螟ｧ螟ｱ雍･</strong><br>' + esc(stage.biggestFailure || '蠕・ｿ占｡・) + '<br><strong>荳倶ｸ髦ｶ谿ｵ驥咲せ</strong><br>' + esc(stage.nextFocus) + '</div></details>'
      ).join('');
      ...
      el('runtime-correction-history').innerHTML = (wr.correctionHistory || []).map(item =>
        '<div class="feed-item"><time>' + esc(item.created_at || '') + '</time><div class="feed-main"><strong>' + esc(item.correction_type || item.target_type || 'correction') + '</strong><span>' + esc(item.workspace || 'JAG-LAB') + ' ﾂｷ ' + esc(item.industry_pack || '') + ' ﾂｷ ' + esc(item.affected_runtime_stage || '') + '</span><span>Reason: ' + esc(item.correction_reason || item.reason || '') + '</span></div><span class="badge b-amber">' + esc(item.status || 'recorded') + '</span></div>'
      ...
      el('runtime-personality-status').innerHTML =
        '<div class="runtime-stat"><span>蠖灘燕莠ｺ譬ｼ</span><strong>' + esc((workspacePersonality.personality || []).join(' / ') || workspacePersonality.voice || 'pending') + '</strong></div>' +
        '<div class="runtime-stat"><span>Platform Personality</span><strong>' + esc(platformPersonality.platform || 'pending') + ' ﾂｷ ' + esc((platformPersonality.style || []).join(' / ') || platformPersonality.tone || '') + '</strong></div>' +
        '<div class="runtime-stat"><span>Market Personality</span><strong>' + esc(marketPersonality.market || 'pending') + ' ﾂｷ ' + esc((marketPersonality.style || []).join(' / ')) + '</strong></div>' +
        '<div class="runtime-stat"><span>Tone Personality</span><strong>' + esc(tonePersonality.tone || 'pending') + ' ﾂｷ ' + esc((tonePersonality.traits || []).join(' / ')) + '</strong></div>' +
        '<div class="runtime-stat"><span>譛菴ｳ莠ｺ譬ｼ</span><strong>' + esc((personality.bestPersonality || {}).tone || 'pending') + '</strong></div>' +
        '<div class="runtime-stat"><span>譛蟶ｸ陲ｫ Reject 逧・ｺｺ譬ｼ</span><strong>' + esc((personality.failedPersonality || {}).tone || 'none') + '</strong></div>' +
        '<div class="runtime-stat"><span>譛霑台ｺｺ譬ｼ貍らｧｻ</span><strong>' + esc(personality.personalityDrift || 'clear') + '</strong></div>';
      ...
      el('runtime-strategy-evolution').innerHTML = (wr.strategyEvolutionFeed || []).map(item =>
        '<div class="feed-item"><time>' + esc(item.platform) + '</time><div class="feed-main"><strong>' + esc(item.classification) + ' ﾂｷ score ' + esc(item.score) + '</strong><span>' + esc(item.strategy) + '</span><span>' + esc(item.reason) + '</span></div><span class="badge ' + (item.classification === 'long_term_growth' ? 'b-green' : 'b-amber') + '">' + esc(item.classification) + '</span></div>'
      ...
      el('runtime-active-patrol-groups').innerHTML = (wr.activePatrolGroups || []).map(item =>
        '<div class="feed-item"><time>' + esc(item.platform) + '</time><div class="feed-main"><strong>' + esc(item.industry_pack) + ' ﾂｷ ' + esc(item.workspace) + '</strong><span>Targets: ' + esc((item.targets || []).join(' / ')) + '</span><span>Keywords: ' + esc((item.keywords || []).join(' / ')) + '</span><span>' + esc(item.next_action || '') + '</span></div><span class="badge b-green">' + esc(item.status || 'active') + '</span></div>'
      ...
      el('runtime-personality-timeline').innerHTML = ((personality.personalityTimeline || []).slice(-8)).map(item =>
        '<div class="feed-item"><time>' + esc(item.type) + '</time><div class="feed-main"><strong>' + esc(item.tone || 'tone') + ' ﾂｷ ' + esc(item.platform || 'platform') + '</strong><span>' + esc(item.reason || '') + '</span></div><span class="badge ' + (item.type === 'failed_personality' ? 'b-amber' : 'b-blue') + '">' + esc(item.type) + '</span></div>'
      ...
      ).join('') || '<div class="correction-card"><strong>Personality Drift</strong><span class="badge b-green">clear</span><p>AGOS 蠖灘燕莠ｺ譬ｼ譛ｪ譽豬句芦譁ｰ蠅樊ｼらｧｻ縲・/p><p>扈ｧ扈ｭ逶第而霑・ｺｦ關･髞縲∬ｿ・ｺｦ諠・ｻｪ蛹悶∝ｹｳ蜿ｰ莠ｺ譬ｼ髞吩ｹｱ縲…lickbait縲∵惻譴ｰ蝗槫､阪∝・螳ｹ驥榊､阪・/p></div>';
      ...
      el('runtime-human-personality-training').innerHTML = ((personalityTraining.events || []).slice(-5)).map(item =>
        '<div class="feed-item"><time>' + esc(item.decision) + '</time><div class="feed-main"><strong>' + esc(item.platform) + ' ﾂｷ ' + esc(item.tone) + '</strong><span>' + esc(item.reason) + '</span></div><span class="badge b-blue">' + esc(item.decision) + '</span></div>'
      ...
      el('runtime-opportunity-ranking').innerHTML = (wr.opportunityRanking || []).map(item =>
        '<div class="feed-item"><time>' + esc(item.verdict) + '</time><div class="feed-main"><strong>' + esc(item.question_id) + ' ﾂｷ score ' + esc(item.total_score) + '</strong><span>' + esc((item.reasons || []).join(' / ')) + '</span></div><span class="badge b-green">隶ｭ扈・愛譁ｭ</span></div>'
      ).join('') || '<div class="feed-item"><time>WAIT</time><div class="feed-main"><strong>遲牙ｾ・ｮｭ扈・ｯ・・</strong><span>Runtime Training 蟆壽悴蜀吝・ opportunityRanking縲・/span></div><span class="badge b-amber">pending</span></div>';
      ...
      ).join('') || '<div class="feed-item"><time>WAIT</time><div class="feed-main"><strong>遲牙ｾ・Intelligence Deposit</strong><span>Best Answer / Best Hook / Failed Strategy 蟆壽悴蜀吝・縲・/span></div><span class="badge b-amber">pending</span></div>';
    ...
    function renderGit() {
      ...
      if (git.latestCommitUrl) {
        ...
        el("git-link").textContent = "謇灘ｼ蠖灘燕 commit";
      }
    }
    ...
    function renderModules() {
      el("module-grid").innerHTML = state.modules.map(item => `
        <article class="card">
          ...
          <p><strong>${Number(item.progress) || 0}%</strong> ﾂｷ <code>${esc(item.ownerPath)}</code></p>
          <p>${esc(item.next)}</p>
        </article>
      `).join("");
    }
    ...
    function previousRoundId(roundId) {
      ...
      return index > 0 ? state.rounds[index - 1].id : "譌";
    }

    function roundActionLabel(round) {
      if (round.status === "done") return "譛ｬ霓ｮ蟾ｲ螳梧・・梧遭隕∝ｺ碑ｮｰ蠖募ｮ樣刔菫ｮ謾ｹ縲・ｪ瑚ｯ∫ｻ捺棡縲；it 迚域悽蜥悟黄菴咎｣朱勦縲・;
      if (round.status === "in_progress") return "譛ｬ霓ｮ豁｣蝨ｨ謇ｧ陦鯉ｼ梧遭隕∫畑莠手ｿｽ雕ｪ蠖灘燕逶ｮ譬・∝ｷｲ螳梧・驛ｨ蛻・∝ｾ・ｪ瑚ｯ・｡ｹ逶ｮ蜥御ｸ倶ｸ豁･蜉ｨ菴懊・;
      if (round.status === "blocked") return "譛ｬ霓ｮ蠖灘燕髦ｻ蝪橸ｼ梧遭隕∝ｺ比ｼ伜・隸ｴ譏朱仆蝪槫次蝗縲∵園髴蜀ｳ遲門柱諱｢螟肴擅莉ｶ縲・;
      return "譛ｬ霓ｮ蟆壽悴蠑蟋具ｼ梧遭隕∬ｯｴ譏主叉蟆・ｻｺ隶ｾ逧・・蜉帙∝・隶ｸ謾ｹ蜉ｨ闌・峩縲・ｪ瑚ｯ∵婿蠑丞柱螳梧・譬・㊥縲・;
    }

    function roundSummary(round) {
      if (round.summary) return round.summary;
      const gateText = isGateRound(round.id)
        ? "霑呎弍髦ｶ谿ｵ鬪梧噺 Round・悟ｮ梧・蜷主ｿ・｡ｻ蛛懈ｭ｢扈ｧ扈ｭ謗ｨ霑幢ｼ悟ｹｶ騾夂衍逕ｨ謌ｷ莠ｺ蟾･鬪梧噺縲・
        : `謇ｧ陦悟燕蠢・｡ｻ蜈磯ｪ瑚ｯ∽ｸ贋ｸ霓ｮ ${previousRoundId(round.id)} 逧・冠蜻翫∵ｵ玖ｯ慕ｻ捺棡縲∵而蛻ｶ荳ｭ蠢・憾諤∝柱 Git 迥ｶ諤√Ａ;
      return `${round.name} 螻樔ｺ・${round.phase}縲よ悽霓ｮ譬ｸ蠢・ｺ､莉俶弍・・{round.deliverable}縲・{roundActionLabel(round)} 鬪瑚ｯ・㍾轤ｹ蛹・峡謚･蜻願ｷｯ蠕・${round.report} 譏ｯ蜷ｦ逕滓・謌匁峩譁ｰ縲∵而蛻ｶ荳ｭ蠢・憾諤∵弍蜷ｦ蜷梧ｭ･縲∫嶌蜈ｳ smoke test 譏ｯ蜷ｦ騾夊ｿ・・{gateText}`;
    }
    ...
    function renderRoundDetail(round) {
      ...
      el("round-detail").innerHTML = `
        <div class="topline">
          <div>
            <h3>${esc(round.id)} ﾂｷ ${esc(round.name)}</h3>
            <p>${esc(roundSummary(round))}</p>
          </div>
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            ${badge(round.status)}
            <button class="detail-close" type="button" aria-label="蜈ｳ髣ｭ Round 鞫倩ｦ・>蜈ｳ髣ｭ</button>
          </div>
        </div>
        <div class="round-detail-grid">
          <div class="detail-box">
            <h3>${round.status === "done" ? "譛ｬ霓ｮ蟾ｲ蛛・ : "譛ｬ霓ｮ隕∝★"}</h3>
            <ul>
              <li>蝗ｴ扈・${esc(round.deliverable)} 蟒ｺ遶句庄鬪瑚ｯ∽ｺ､莉倡黄縲・/li>
              <li>謇ｧ陦悟燕譽譟･荳贋ｸ霓ｮ扈捺棡・梧鴬陦悟錘譖ｴ譁ｰ謗ｧ蛻ｶ荳ｭ蠢・柱 Round 謚･蜻翫・/li>
              <li>${isGateRound(round.id) ? "譛ｬ霓ｮ譏ｯ髦ｶ谿ｵ鬪梧噺轤ｹ・碁怙隕∫畑謌ｷ遑ｮ隶､蜷主・扈ｧ扈ｭ縲・ : "譛ｬ霓ｮ荳肴弍髦ｶ谿ｵ髣ｨ・悟ｮ梧・鬪瑚ｯ∝錘蜿ｯ莉･霑帛・荳倶ｸ霓ｮ縲・}</li>
            </ul>
          </div>
          <div class="detail-box">
            <h3>鬪瑚ｯ∽ｸ取冠蜻・/h3>
            <ul>
              <li>蜑咲ｽｮ鬪瑚ｯ・ｼ・{esc(previousRoundId(round.id))}</li>
              <li>謚･蜻願ｷｯ蠕・ｼ・code>${esc(round.report)}</code></li>
              <li>霑帛ｺｦ・・{Number(round.progress) || 0}%</li>
              <li>鬪梧噺・・{isGateRound(round.id) ? "髴隕∫畑謌ｷ髦ｶ谿ｵ鬪梧噺" : "蜊丈ｽ憺ｪ梧噺隶ｰ蠖募・蜈･謚･蜻・}</li>
            </ul>
          </div>
        </div>
      `;
    ...
    function renderRounds(filter = "all") {
      ...
      el("round-list").innerHTML = rounds.map(r => `
        <article class="round-card" data-round-id="${esc(r.id)}" tabindex="0" role="button" aria-label="譟･逵・${esc(r.id)} 鞫倩ｦ・>
          <div class="round-top">
            <h3 class="round-title">${esc(r.id)} ﾂｷ ${esc(r.name)}</h3>
            ${badge(r.status)}
          </div>
          <div class="bar" style="--value:${Number(r.progress) || 0}%"><i></i></div>
          <ul>
            <li>${esc(r.phase)}</li>
            <li>莠､莉倡黄・・{esc(r.deliverable)}</li>
            <li>謚･蜻奇ｼ・code>${esc(r.report)}</code></li>
          </ul>
        </article>
      `).join("");
    ...
    function renderReports() {
      el("report-list").innerHTML = state.reports.map(report => `
        <article class="card" style="margin-bottom:12px">
          <div class="topline">
            <h3>${esc(report.id)} ﾂｷ ${esc(report.title)}</h3>
            ${badge(report.status)}
          </div>
          <p>${esc(report.date)} ﾂｷ ${esc(report.summary)}</p>
          <div class="table-wrap">
            <table>
              <tbody>
                <tr><th>菫ｮ謾ｹ譁・ｻｶ</th><td>${report.files.map(f => `<code>${esc(f)}</code>`).join(" ")}</td></tr>
                <tr><th>鬪瑚ｯ∫ｻ捺棡</th><td>${report.verification.map(v => esc(v)).join("・・)}</td></tr>
              </tbody>
            </table>
          </div>
        </article>
      ...
      el("version-list").innerHTML = state.versions.map((version, index) => `
        <div class="version-row">
          <div>
            ...
            <p>${esc(version.date)} ﾂｷ Git: <code>${esc(version.gitRef)}</code> ﾂｷ 菴咲ｽｮ・・code>${esc(version.location)}</code></p>
            <p>${esc(version.notes)}</p>
          </div>
          <div class="version-actions">
            <button class="action primary" data-version="${index}">菫晏ｭ伜ｿｫ辣ｧ</button>
            <button class="copy-btn" data-copy="${esc(version.location)}">螟榊宛菴咲ｽｮ</button>
          </div>
        </div>
      `).join("");
    ...
    function renderPaths() {
      el("path-table").innerHTML = state.paths.map(item => `
        <tr>
          ...
          <td><button class="copy-btn" data-copy="${esc(item.path)}">螟榊宛霍ｯ蠕・/button></td>
        </tr>
      `).join("");
